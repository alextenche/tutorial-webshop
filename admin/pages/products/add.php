	...
	if($objValid->isValid()){
		...
		if($objCatalogue->addProduct($objValid->_post)){
			...
			if($objUpload->upload(CATALOGUE_PATH)){
				...
				Helper::redirect('/ecommerce/admin'.Url::getCurrentUrl(array('action','id')).'&action=added-no-upload');
			}
		} else {
			Helper::redirect('/ecommerce/admin'.Url::getCurrentUrl(array('action','id')).'&action=added-failed');
		}
	}
}
 ?>
	...
	<table cellpadding="0" cellspacing="0" border="0" class="tbl_insert">
		...
		<tr>
			...
			<td>
				...
				<?php echo $objForm->stickyText('description'); ?>
				</textarea>
			</td>
		</tr>

...
if(!empty($id)){
	...
	if(!empty($order)) {
		...
		if($objForm->isPost('status')){
			...
			$objValid->_expected = array('status','note');
			...
			if($objValid->isValid()){
				...
				if($objOrder->updateOrder($id, $vars)){
					Helper::redirect('/ecommerce/admin'.Url::getCurrentUrl(array('action','id')).'&action=edited');
					} else {
					Helper::redirect('/ecommerce/admin'.Url::getCurrentUrl(array('action','id')).'&action=edited-failed');
				}
			}
		}
		?>
...
<form action="" method="post">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="tbl_insert">
		...
		<tr>
			...
			<td colspan="4" class="ta_r"><strong>&pound;<?php echo number_format($order['vat'],2);?></strong></td>
		</tr>
		
		<tr>
			<th>Total:</th>
			<td colspan="4" class="ta_r">&pound;<?php echo number_format($order['total'],2);?></td>
		</tr>
		
		<tr>
			<th>Client:</th>
			<td colspan="4">
				<?php echo Helper::encodeHtml($user['first_name']." ".$user['last_name']).'<br />';
		...
		<tr>
			<th>PP status:</th>
			<td colspan="4"><?php echo !empty($order['payement_status']) ?
								  Helper::encodeHtml($order['payement_status']) :
								  "Pending";?>
			</td>
		</tr>
		...
		<tr>
			...
			<td colspan="4">
				<textarea name="notes" id="notes" cols="" rows="" class="tar"><?php echo $objForm->stickyText('notes',$order['notes']);?></textarea>
			</td>
		</tr>
		...
		<tr>
			...
			<td colspan="4">
				...
				<div class="sbm sbm_blue fl_r">
					<a href="<?php echo '/ecommerce/admin'.Url::getCurrentUrl(array('action')).'&action=invoice'; ?>" class="btn" target="_blank">Invoice</a>
				</div>
				
				<div class="sbm sbm_blue fl_l mr_r4">
					<a href="<?php echo '/ecommerce/admin'.Url::getCurrentUrl(array('action','id')); ?>" class="btn" >Go back</a>
				</div>

...
$objPaging->_url = '/ecommerce/admin'.$objPaging->_url;
...
<form action="" method="get">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="tbl_insert">
		...
		<tr>
			...
			<td>
				<input type="text" name="srch" value="<?php echo stripslashes($srch);?>" class="fld" />
			</td>
			<td>
...
<table cellpadding="0" cellspacing="0" border="0" class="tbl_repeat">
	...
	<tr>	
		...
		<td class="ta_r">
			<?php $status = $objOrder->getStatus($order['status']);
			echo $status['name'];?>
		</td>
		<td class="ta_r">
			<?php echo $order['payment_status'] != null ? $order['payment_status'] : "Pending"; ?>
		</td>
		<td class="ta_r">
			<?php if($order['status'] == 1) { ?>
				<a href="/ecommerce/admin/?page=orders&amp;action=remove&amp;id=<?php echo $order['id'];?>">Remove</a>
			<?php } else { ?>
			<span class="inactive">Remove</span>
			<?php } ?>
		</td>	
		<td class="ta_r"><a href="/ecommerce/admin/?page=orders&amp;action=edit&amp;id=<?php echo $order['id'];?>">View</a></td>
	</tr>
